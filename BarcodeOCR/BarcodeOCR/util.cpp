...
std::vector<std::string> Utils::getFiles(const std::string &folder,
  ...
  subfolders.push_back(folder);//将folder加入容器subfoders的最后一位
#ifdef OS_WINDOWS
  while (!subfolders.empty()) {
    std::string current_folder(subfolders.back());//返回容器subfolder的最后一个元素的值
    ...
    subfolders.pop_back();//删除subfolder最尾端的元素

	//struct _finddata_t 是用来存储文件各种信息的结构体:包含文件属性：隐藏，只读等性；文件创建时间；
	//文件最后一次被访问的时间；文件最后一次被修改的时间；文件的大小；文件名
    struct _finddata_t file_info;

	//findfirst，搜索与指定的文件名称匹配的第一个实例，若成功则返回第一个实例的句柄，否则返回-1L。
    auto file_handler = _findfirst(current_folder.c_str(), &file_info);

	//strcmp:比较两个字符串，若字符串相等则返回零；
